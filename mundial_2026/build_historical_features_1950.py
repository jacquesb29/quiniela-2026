...
DEFAULT_OUTPUT_FILE = ROOT / "historical_features_1950.json"
DEFAULT_START_DATE = "1950-01-01"
DEFAULT_MIN_OFFICIAL_MATCHES = 25000
TODAY = date.today()
...
def resolve_source_path(value: str | None, url: str, target_dir: Path, download: bool) -> Path:
    if value:
        return Path(value)
    if not download:
        raise SystemExit(f"Falta ruta local para {Path(url).name}; usa --download o proporciona el archivo.")
    return download_source(url, target_dir)
...
def build_output_record(team: str, aliases: list[str], bucket: dict[str, float], shootouts: dict[str, float]) -> dict[str, object]:
    ...
    return {
        ...
        "matches_since_start": int(matches),
        "weighted_matches_since_start": round(weighted_matches, 3),
        ...
        "competitive_matches_since_start": int(competitive_matches),
        ...
        "world_cup_matches_since_start": int(world_cup_matches),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Construye features históricos trazables de selecciones desde 1950.")
    parser.add_argument("--teams", default=str(DEFAULT_TEAMS_FILE))
    parser.add_argument("--results")
    parser.add_argument("--shootouts")
    parser.add_argument("--former-names")
    parser.add_argument("--output", default=str(DEFAULT_OUTPUT_FILE))
    parser.add_argument("--start-date", default=DEFAULT_START_DATE)
    parser.add_argument("--min-official-matches", type=int, default=DEFAULT_MIN_OFFICIAL_MATCHES)
    parser.add_argument("--download", action="store_true")
    args = parser.parse_args()

    with tempfile.TemporaryDirectory(prefix="worldcup-history-") as tmpdir:
        source_dir = Path(tmpdir)
        results_path = resolve_source_path(args.results, DATASET_SOURCE_RESULTS, source_dir, args.download)
        shootouts_path = resolve_source_path(args.shootouts, DATASET_SOURCE_SHOOTOUTS, source_dir, args.download)
        former_names_path = resolve_source_path(args.former_names, DATASET_SOURCE_FORMER_NAMES, source_dir, args.download)

        teams = load_teams(Path(args.teams))
        team_aliases = build_team_name_map(teams, former_names_path, args.start_date)
        ...
        source_official_match_count = 0
        skipped_unplayed_match_count = 0
        ...
        with results_path.open(newline="") as handle:
            for row in csv.DictReader(handle):
                ...
                try:
                    ...
                    skipped_unplayed_match_count += 1
                    continue
                source_match_count += 1
                tournament = row["tournament"]
                if tournament not in COMPETITIVE_TOURNAMENT_EXCLUDE:
                    source_official_match_count += 1
        ...
        if source_official_match_count < args.min_official_matches:
            raise SystemExit(
                f"Histórico insuficiente: {source_official_match_count} partidos oficiales "
                f"desde {args.start_date}; mínimo requerido={args.min_official_matches}."
            )

        shootouts = defaultdict(lambda: {"matches": 0.0, "wins": 0.0})
        with shootouts_path.open(newline="") as handle:
            for row in csv.DictReader(handle):
                if row["date"] < args.start_date:
                    continue
                participants = [
        ...
        for team in [entry["name"] for entry in teams]:
            ...
            team_rows[team] = row
            world_cup_experience = min(float(row["world_cup_matches_since_start"]) / 20.0, 1.0)
            shootout_experience = min(float(row["shootout_matches_since_start"]) / 12.0, 1.0)
            strength_raw[team] = (
        ...
        output = {
            "meta": {
                ...
                "source_matches_since_start": source_match_count,
                "official_matches_since_start": source_official_match_count,
                "skipped_unplayed_matches_since_start": skipped_unplayed_match_count,
                "minimum_official_matches_required": args.min_official_matches,
                "official_match_definition": "Partidos internacionales cerrados cuyo torneo no es Friendly ni Unofficial Friendly.",
        ...
        print(
            f"Escrito: {output_path} | oficiales={source_official_match_count} "
            f"| total={source_match_count} | desde={args.start_date}"
        )
