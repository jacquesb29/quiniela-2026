...
def penalties_probability(
    ...
    *,
    ...
    edge += 0.22 * (
        getattr(profile_a.squad, "penalty_granular_index", 0.5)
        - getattr(profile_b.squad, "penalty_granular_index", 0.5)
    )
    edge += 0.12 * (
        getattr(profile_a.squad, "goalkeeper_penalty_save_rate", 0.5)
        - getattr(profile_b.squad, "goalkeeper_penalty_save_rate", 0.5)
    )
...
def penalty_conversion_probability(
    ...
    *,
    ...
    rate += 0.07 * centered(getattr(taker_profile.squad, "penalty_taker_quality", 0.5))
    rate += 0.04 * centered(getattr(taker_profile.squad, "shootout_pressure_experience", 0.5))
    rate += 0.02 * clamp(getattr(taker_profile.squad, "keeper_taker_strategy_matchup", 0.0), -1.0, 1.0)
    ...
    rate -= 0.07 * centered(getattr(keeper_profile.squad, "goalkeeper_penalty_save_rate", 0.5))
    rate -= 0.02 * clamp(getattr(keeper_profile.squad, "keeper_taker_strategy_matchup", 0.0), -1.0, 1.0)
